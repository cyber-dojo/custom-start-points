...
readonly MERKELY_PIPELINE=custom-start-points


# - - - - - - - - - - - - - - - - - - -
kosli_fingerprint()
{
  echo "docker://${CYBER_DOJO_CUSTOM_START_POINTS_IMAGE}:${CYBER_DOJO_CUSTOM_START_POINTS_TAG}"
}
...
kosli_log_artifact()
{
  ...
  docker run \
    ...
    --env MERKELY_ARTIFACT_GIT_COMMIT=${CYBER_DOJO_CUSTOM_START_POINTS_SHA} \
    --env MERKELY_ARTIFACT_GIT_URL=https://github.com/${MERKELY_OWNER}/${MERKELY_PIPELINE}/commit/${CYBER_DOJO_CUSTOM_START_POINTS_SHA} \
